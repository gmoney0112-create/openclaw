...
export function mergeScopedSearchConfig(
  ...
  const currentScopedRaw = searchConfig?.[key];
  const currentScoped =
    currentScopedRaw && typeof currentScopedRaw === "object" && !Array.isArray(currentScopedRaw)
      ? (currentScopedRaw as Record<string, unknown>)
      : {};
